...
function generarCalendario(id_paciente){
    return new Promise((resolve, reject) =>{
        apiRequest("http://127.0.0.1:8000/api/citas/paciente/" + id_paciente, (err, response, body) => {
            if (!err && response.statusCode == 200) {
                citas = JSON.parse(body); // asignamos el JSON a pacientes
                citasParsed = []
                for(var i =0; i<citas.length; i++) {
                    citasParsed[i]={
                        title: citas[i]['id_doctor']['nombre'] + " " + citas[i]['id_doctor']['apellido'] + " Ficha: " + citas[i]['id_cita'],
